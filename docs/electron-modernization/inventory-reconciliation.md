...
## September 25 bounded image-save and deployment composition

Original modernization baseline: `1b82b085ac1436a7f21d81cb944d2ee2f4ba4a4a`. Current composed source through `547cdca5` adds SEC-008 F-037 bounded preload streaming and PKG-002 F-040 strict release artifact resolution after separate failing test baselines. Compared with the prior 553-row candidate, exactly two test paths newly enter the original-baseline delta: `electron/src/preload/menu/preload-context.test.main.ts` (A) and `bin/deploy-tools/lib/deploy-utils.test.ts` (M). `git diff --no-renames --name-status <baseline> 547cdca5` matches **555 unique path/status rows** in `review-inventory.csv`: no missing, extra, duplicate or status mismatch. Current exact `pending` cells are **152 source review, 239 provenance and 341 coverage disposition**, plus many partial cells. Most remaining source-review placeholders are test/fixture paths (126); one is `yarn.lock`, and 25 are documentation. This is path accounting and bounded findings work, **not** a completed holistic review or final platform acceptance. Current-head CI has not yet run for these two source fixes; the prior `ba575034` Windows/Linux package success, macOS native lifecycle failure and E2E macOS retries are historical evidence only.

## September 25 SEC-008/SEC-003/media composed source accounting

The original baseline remains `1b82b085ac1436a7f21d81cb944d2ee2f4ba4a4a`. The current isolated TST-006 application source is `c832da76`, with test-only startup fixture correction `644a3dc3` and signed-pipeline correction `26979240`: the four-fix application, the SEC-008 F-035 foreign-child baseline and one-use popup grant, the SEC-003 F-036 failing diagnostic baseline and fixed proxy preload, and the bounded TST-005 macOS media case. This branch also carries the September 24 audit notes and the later audit findings. The delta adds exactly five new production/test paths to the earlier 548-row ledger, all for the popup grant; existing paths retain their original changed statuses. `git diff --no-renames --name-status <baseline> c832da76` matches **553 unique path/status rows**, with no missing, extra, duplicate or status mismatch. Exact placeholder counts after bounded SEC-008/SEC-003 source notes are **170 source-review, 239 test-provenance and 341 coverage-disposition pending**; many populated cells remain partial. Separate scoped [SEC-008 three-platform run `36068535525`](https://github.com/adamlow-wire/wire-desktop/actions/runs/36068535525) passes, while SEC-003 [scoped run `36070507112`](https://github.com/adamlow-wire/wire-desktop/actions/runs/36070507112) and [exact final-composition run `36071785851`](https://github.com/adamlow-wire/wire-desktop/actions/runs/36071785851) have Linux/macOS full jobs green; the scoped Windows job failed near one hour without an installed-smoke completion marker/log, and the composed Windows job failed near one hour with the same missing completion marker. Both Windows installed-app checks and final-head E2E remain unqualified. This is path accounting, not a completed holistic review or a final handoff claim.

[Later same-source package run `36076780279`](https://github.com/adamlow-wire/wire-desktop/actions/runs/36076780279) passes complete Windows/Linux jobs, including installed Windows Squirrel/MSI smoke, but macOS stops before packaging in the previously documented native zero-exit fixture with a flaky retry and worker-teardown error. [E2E/report `36076780299`](https://github.com/adamlow-wire/wire-desktop/actions/runs/36076780299) completes all three OS jobs yet retains two macOS API-client-403 first-attempt failures and passing retries. Neither alters the 553-path accounting or qualifies the later F-037/F-040 source fixes.

## September 24 exact four-fix composed source accounting

The original baseline remains `1b82b085ac1436a7f21d81cb944d2ee2f4ba4a4a`. The reviewed application/source composition is `0fd4679a2cb5d100bdb9d856627903b49bd4bcd8`: PR #63 plus the scoped PR #66/#67/#68/#69 source and tests, with the SEC-003 native test selector. This separate TST-006 audit branch adds the source-review and upstream-delta notes without changing application source. Compared with the previous 543-row audit ledger, five newly changed paths were inserted: the composed checkpoint, both desktop-config source/test files, managed constants, and the native frame-lifecycle test. The two review notes stay in the ledger because this audit branch adds them. `git diff --no-renames --name-status <baseline> <audit branch>` matches **548 unique path/status rows** with no missing, extra, duplicate or status mismatch. Exact placeholder counts are now **259 source-review, 330 test-provenance and 336 coverage-disposition pending** after the binder/preload/popup initial source passes, focused 29/23-case Node runs and the separate F-035 native baseline; many populated cells are also partial. This is path accounting, not a completed holistic review or proof of full coverage. [Composed native/package run `36058028533`](https://github.com/adamlow-wire/wire-desktop/actions/runs/36058028533) is on exact application SHA `0fd4679a`: Linux has passed; Windows passes the full job including selected native frame test and installed Squirrel/MSI smoke; macOS stopped before packaging in the second fake-media case on a two-second body timeout before its legacy-capture denial assertion. This is incomplete qualification, not a demonstrated policy bypass. The earlier `1f999595` run passed Linux/macOS but its Windows job was cancelled by the workflow's same-branch concurrency when the corrected run started; no Windows installer outcome from that earlier SHA is accepted.
