...
The original baseline remains `1b82b085ac1436a7f21d81cb944d2ee2f4ba4a4a`. The current isolated TST-006 final-composition source is `c832da76`: the four-fix application, the SEC-008 F-035 foreign-child baseline and one-use popup grant, the SEC-003 F-036 failing diagnostic baseline and fixed proxy preload, and the bounded TST-005 macOS media case. This branch also carries the September 24 audit notes and the later audit findings. The delta adds exactly five new production/test paths to the earlier 548-row ledger, all for the popup grant; existing paths retain their original changed statuses. `git diff --no-renames --name-status <baseline> c832da76` matches **553 unique path/status rows**, with no missing, extra, duplicate or status mismatch. Exact placeholder counts after bounded SEC-008/SEC-003 source notes are **231 source-review, 300 test-provenance and 341 coverage-disposition pending**; many populated cells remain partial. Separate scoped [SEC-008 three-platform run `36068535525`](https://github.com/adamlow-wire/wire-desktop/actions/runs/36068535525) passes, while SEC-003 [scoped run `36070507112`](https://github.com/adamlow-wire/wire-desktop/actions/runs/36070507112) and [exact final-composition run `36071785851`](https://github.com/adamlow-wire/wire-desktop/actions/runs/36071785851) have Linux/macOS full jobs green and Windows installed-app smoke still in progress. Both Windows jobs and final E2E remain unqualified. This is path accounting, not a completed holistic review or a final handoff claim.
