...
Native transport evidence confirms that relative resources need pre-ready standard-scheme registration, consistent with [Electron's protocol documentation](https://www.electronjs.org/docs/latest/api/protocol). The production shell cutover passes the existing Linux lifecycle and metadata/restart fixtures after its new scheme assertion fails against `file:`. Auxiliary cutover, custom-scheme ordinary-script CSP and final platform qualification remain open; this ADR is not yet accepted.
