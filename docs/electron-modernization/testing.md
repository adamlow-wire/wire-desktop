...
TST-006 media fixture setup review (September16): repeated Windows runs35034540782/job104600525641 and35035452235/job104603441247 fail the native media beforeEach at Mocha's implicit two-second limit, before any media request. Setup now uses the existing account-fixture ten-second budget, with fixed stage/duration diagnostics after1.5 seconds. Media test bodies retain two seconds; native fake-device guards, consent assertions and production timeouts are unchanged. This addresses an evidenced fixture-budget mismatch, not the separate macOS media-body timeouts or Windows session-clear stall. Hosted qualification remains required.
