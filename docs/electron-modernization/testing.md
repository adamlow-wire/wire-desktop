...
`bin/test-tools/packaged-account-smoke.cjs` is a compatibility/security-target fixture for DCP-013 and page Node denial. CI passes the actual packaged executable with unchanged fuses. It uses normal account navigation to an isolated nonce-bearing loopback page; no debugging port, inspector, injected preload or production test mode is involved. It asserts an account ID, the compatibility bridge, immutable exact App-lock configuration, absent page Node globals and top-level account context. On macOS/Linux, a second fresh launch reads a real test-owned OS policy value, proving the configuration changes on restart. Windows uses an explicit managed fixture for ordinary and authenticated-proxy startup because hosted runners may already be enrolled; it does not claim an unmanaged device or a false-to-true transition. Windows/macOS launches are restricted to disposable GitHub Actions runners; Linux profile/XDG state is isolated. Existing machine policy is not overwritten; workflow traps/finally remove only the created value. This is backend compatibility, not MDM enrollment/deployment. The managed launch also exercises an authenticated HTTP proxy as described below. Cleanup terminates only the fixture's child tree and does not claim graceful shutdown.
...
CAP-005 retained-policy characterization under resolved Q-005 covers built-in enabled default, no-selection denial, explicit native override with triggering-request denial and subsequent Chromium delegation, process-wide scope, mandatory Chromium-error denial, irrelevant Chromium-warning checkbox rejection and fresh module initialization. Fresh-module state is not claimed as an actual process restart. Temporarily conditioning Chromium error handling on pinning being enabled makes the targeted test fail with -3 instead of -2; restore source before all validation. Initial27 certificate cases/types/lint pass. Logs `/tmp/m3-pinning-policy-{build,types,lint,native,sensitivity}.log`. No runtime implementation or user-facing policy changes.
...
All-M3 comparison uses `DIFF_COVERAGE_BASE=fe0b86cbd05a49ef84e785478f7e32995d161792` (completed M2 ancestor) against fresh full coverage. The original git diff invocation fails ENOBUFS on1,452,367 bytes. Raising its bounded maxBuffer to16MiB preserves the command, source roots, parsers and80%/90% thresholds; the same reports pass3174/3758 changed statements84.46% and840/853 tracked security branches98.48%. The default-buffer failure is retained in `/tmp/m3-all-changes-coverage.log`; restored-capacity result `/tmp/m3-all-changes-diff-coverage.log`. This is an aggregate audit, not blanket proof of every security property. Full `yarn test` also passes112 Jest/901 main/4 media/4 renderer/38 build-tool cases (`/tmp/m3-all-changes-test.log`).

CAP-002 closeout: reviewed43961f6c merged as3c734cde in [PR #53](https://github.com/adamlow-wire/wire-desktop/pull/53) after [native34901989430](https://github.com/adamlow-wire/wire-desktop/actions/runs/34901989430) and [full E2E/report34901989437](https://github.com/adamlow-wire/wire-desktop/actions/runs/34901989437) plus build/lint/analysis passed. Windows native third attempt passes173 account/465 boundary/nine product cases; retain initial137-pass/seven-timeout and second172-pass/one-startup-timeout failures. No assertions or timeouts changed. Full suites pass46 each: Windows43 initial/three login-readiness retries; macOS42 initial/four retries (two authenticated-login waits, post-clear-data welcome and add-account login button). No skips/worker errors; report104180633497 passes. All new transport cases pass. These flakes remain owned TST-005 follow-up, not ignored failures. Logs `/tmp/m3-cap002-{windows,macos}-e2e-final.log` and `/tmp/m3-cap002-windows-native-final.log`; read exact job logs directly because old gh caches can return a prior run attempt.

## September15 final-audit credential diagnostics (CAP-006 / supporting CAP-005)

The final INV-010 source audit found credential payloads in native action dispatch, settings save/read/persist and the stored-proxy startup diagnostic. Separate baseline3df23657 fails both SSO/join targets; baselineef202b1d fails all three isolated settings operations and the real startup-log fixture. The settings tests use a test-owned file and preserve/restore the singleton path and global configuration. No real user profile is used.

The focused correction removes only diagnostic payloads, retaining action/window and operation metadata. Tests assert exact immediate/queued delivery, real settings round trips and the unchanged startup proxy switch. `yarn test:main --grep 'ConfigurationPersistence diagnostics|WindowManager queued actions|dispatchDeepLink|deep-link policy'` passes47 cases. TypeScript/Mocha types and scoped lint pass. TypeScript build then `bundle:dev` precede `test:e2e --project=macOS e2e-tests/specs/regression/proxySession.spec.ts`, which passes four product cases in10.1s on Linux with isolated profiles/keyring. The project name is not macOS platform evidence.

Restoring old action payloads fails both targets; restoring old settings payloads fails all three. Restoring only the compiled startup URL diagnostic fails its independent target, so configuration redaction alone cannot hide that leak. After restoration the product target passes in1.3s and all47 native targets pass again in182ms. All perturbations are restored. The all-platform native boundary selection includes the new settings tests; proxy startup already runs in each product gate and full E2E. Final-head hosted qualification and whole-M3 aggregate validation remain mandatory.

## CAP-001 startup redirect regression (September 15)

The retained macOS account-login retry in [PR #55 E2E](https://github.com/adamlow-wire/wire-desktop/actions/runs/34909605643) exposed a production startup race. With an initial resource pending, a same-origin client redirect rejects Electron's initial `loadURL` promise with `ERR_ABORTED` and the old account owner destroys the view. A deterministic native fixture reproduces one failure/one deny-path pass before implementation (baseline `3fe42493`). A separate real product-shell fixture (`06d8b5aa`) fails to obtain the surviving account page against the old compiled implementation. Both fixtures use only their own loopback servers and profiles.

The correction observes only main-frame document replacements and requires an authorized same-origin destination, current native ownership, document completion and stopped loading. Ordinary failures, foreign navigation, stale ownership, destruction and crashes remain failures. A replacement that never completes has a bounded 30-second wait; owned listeners and timers are always removed. Restoring the original `AccountViews` load reproduces teardown. Removing the origin check fails the foreign-commit target; removing current ownership fails the stale-owner target. All perturbations are restored before validation.

The new native tests are included by the existing account test glob, and the product fixture is included in the existing all-platform navigation selection and full E2E suite. Final-head aggregate and hosted results are recorded with the scoped startup PR; previous-head passes do not qualify this implementation. Live provider/ACME qualification remains downstream QA.

The final error-path review also reproduces credential exposure from Electron's native load-error message, which embeds the destination URL. A separate sensitive baseline requires failure without the synthetic secret or URL. Startup failures now retain only a bounded native `ERR_*` code and generic operation context, never the native message or cause; rejection and cleanup remain mandatory. This follows INV-010 and does not alter navigation acceptance.

## Final M3 aggregate and platform acceptance

[PR #56](https://github.com/adamlow-wire/wire-desktop/pull/56) qualifies `5c596187`, merged tree-equal as `45840fca`. Complete local tests pass 112 Jest/923 main/4 media/4 renderer/38 build-tool cases, types/build, and six rebuilt Linux navigation/proxy product cases. Whole-M3 coverage from completed M2 `fe0b86cb` passes 3245/3830 changed statements (84.73%) and 840/853 tracked security branches (98.48%). [Native34913964577](https://github.com/adamlow-wire/wire-desktop/actions/runs/34913964577) passes every platform initially; [E2E/report34913964586](https://github.com/adamlow-wire/wire-desktop/actions/runs/34913964586) passes 48 cases each, macOS without retries and Windows with one retained fixture-login retry. No skipped test or worker error counts as a pass. See [m3-acceptance.md](m3-acceptance.md) for the full audit, commands, retry diagnosis and explicit downstream QA/release limits.

## Functional M4 platform qualification

TST-005 adds an explicit Linux development E2E project alongside Windows/macOS. Modernization PRs and integration pushes schedule all three; branch-protection enforcement and hosted final-head evidence remain acceptance gates. The private Linux runner uses a test-owned native Secret Service and temporary DBus/XDG directories, never plaintext storage. Native package smoke remains separate and is not replaced by development E2E.

Fixture setup now rejects unsuccessful mutation responses, creates valid handles independently of display names and attempts cleanup of partial accounts/teams. Cleanup errors remain visible alongside the setup failure. Existing retries remain bounded at two on CI; no test is quarantined or skipped by this change. TST-005 owns the PR56 Windows missing-handle retry and must verify the corrected fixture in hosted E2E.
...
`test:renderer:coverage` additionally loads `electron/test/collect-renderer-coverage.cjs` as a coverage-only Mocha file. Its root teardown saves the Babel-instrumented renderer counters into nyc's configured temporary directory before electron-mocha reports completion and closes the window. Previously those executions were absent from the report even though the renderer tests passed; native process counters alone do not prove renderer coverage. The collector fails if instrumentation produces no counters. A clean `yarn coverage` recreates the reports and exercises collection without relying on earlier run files. No threshold or production preload is relaxed for coverage.

Local sensitivity removes the main source-selection approval check, removes the frame dimension cap, and independently removes the renderer width bound/replaces source-label text with HTML. Each intended target fails, all perturbations are restored, and the restored suites pass. Current local evidence is recorded in status.md; hosted final-head qualification remains mandatory.

### Persisted service-worker startup under automation

TST-005 pins a narrow Yarn patch to `playwright-core@1.60.0`. Chromium attaches a persisted service worker both to the browser session and to the starting page. Playwright buffers page target attachments until `Page.getFrameTree`; a paused worker can prevent that initial frame tree from completing. The patch lets only redundant page-owned `service_worker` attachments reach the existing detach path immediately. Browser-owned worker instrumentation and iframe/dedicated-worker handling remain intact. The application, account partitions, service workers and permission policy are unchanged.

The owned loopback regression installs a worker, proves its response controls a document, quits natively, reopens the same profile with tracing, and requires the worker-produced title and controller again. Chromium may issue a parallel network request during startup, so request-count absence is not the contract; the received document and controller are. Initial baseline `e5b889e8` reproduces the startup stall. The installed patch passes all six launcher cases (`/tmp/m4-worker-patch-all.log`). Removing only the patch passes once individually, then exposes one initial-document failure in three attempts (`/tmp/m4-worker-patch-sensitivity-repeat.log`); the race is timing-dependent. The patch is restored and three unchanged attempts pass in14 seconds (`/tmp/m4-worker-patch-restored-repeat.log`). Application/E2E types and scoped lint pass. Keep this product regression when updating Playwright; remove the patch only after the unpatched replacement passes it and every supported-platform gate. The patch is development tooling only and does not disable service workers or expose debugger authority to product content.

### Native quit failure cleanup

The owned proxy fixture must retain its ten-second zero-exit assertion and still close its app, servers and temporary profile if that assertion fails. TST-005 baseline `6ddc1283` vetoes one native Quit and observes the process still alive after the assertion rejects. `quitApp` now always attempts native cleanup, rethrows the original assertion and aggregates a simultaneous close failure; nested fixture cleanup also closes servers and removes the profile. The original local21-case composition had20 passes, one native-exit failure and one worker error. The corrected11-case launcher/proxy selection passes without retries. Temporarily restoring the old helper fails the target, and the restored correction passes (`/tmp/m4-quit-cleanup-{sensitivity,final-target}.log`). This does not classify or waive the original shutdown delay. Logs: `/tmp/m4-quit-cleanup-{baseline,restored,types,lint}.log`.

Functional M4 final composition adds two fixture diagnostics without changing product policy: registration failures expose only HTTP status, and native restart judges an explicit Quit by the unchanged ten-second zero-exit check even if its inspector reply is lost. A failed request plus absent exit preserves both failures before owned cleanup. Separate baselines `b3499b86` and `14c74b0c` precede corrections; temporary removal exposes the expected status/restart failures, restoration passes all 15 account-fixture and 13 launcher/proxy cases. The original standalone synchronous exit probe passed because it did not force the race; its replacement leaves the inspector reply pending until actual exit and reproduces the hosted error. See `/tmp/m4-registration-status-*.log` and `/tmp/m4-quit-reply-*.log`. No extra retry, timeout or production security exception is introduced.

## Functional M4 acceptance checkpoint

Reviewed41066868 passes complete build/tests (112 Jest, 964 native, 4 media, 23 renderer, 51 build tools), types/lint/CodeQL, all native/package platforms and [full E2E/report 34986539616](https://github.com/adamlow-wire/wire-desktop/actions/runs/34986539616). Each full platform completes 76 cases: Linux/macOS initially, Windows 75 initially plus one group-member selection retry. Native macOS passes one reviewed unchanged-head rerun after the retained two-second notification-denial timeout; Windows/Linux native pass initially. No skipped test or independent worker error is accepted. The [functional audit](m4-acceptance.md) records criterion mapping and remaining OS/packaging/enterprise QA.

Separate650a00c7/00435dec baseline/fix removes reproducible registration-address collisions using the existing random fixture identifier. All16 API-only fixture cases and removal/restoration checks pass; the particular hosted HTTP409 response body was not retained, so its precise cause remains unasserted. The preceding Windows notification failure reproduces on all three attempts: the second account moves from Distraction Group into the new MLS conversation before the measured message. The corrected fixture first establishes and reads that conversation, then selects/asserts the distraction group and zero unread. It retains all original badge and exact account/conversation activation assertions and passes initially on every platform. No production behavior, timeout or retry is changed.

The maintainer prohibits GUI/native application tests on the shared display after they interrupted their work. Continue hosted CI or truly isolated execution; local API-only fixtures, types and lint remain suitable. The [Windows preview](manual-windows-preview.md) supports manual functional checks alongside installed Wire with a separate profile and distinct identity. Its unsigned packaged smoke does not complete release qualification. The final documentation head must still pass its applicable protected checks before merge.
...
## Windows Squirrel updater diagnostic boundary

PKG-002 F-030 keeps the installed Squirrel updater feed and process output out of application diagnostics. The Node-only direct-source suite executes the actual `spawn`, `spawnUpdate`, `installUpdate` and scheduler functions with a synthetic credential-bearing feed and child process; it checks unchanged arguments to Update.exe, absence of the synthetic secret in logs, nonzero/synchronous/native process failures, and contained scheduled rejection. Run `yarn mocha --require .babel-register.js bin/test-tools/squirrel-diagnostics.test.ts` plus `yarn build:ts`, `yarn build:ts:bin`, `yarn build:ts:tests` and scoped lint. The three targets failed before `11df0aae`; four cases pass after it. In-memory logging and nonzero-exit perturbations fail their corresponding tests, then unchanged source passes again. No real updater, feed, Squirrel installation or Windows GUI was exercised locally; hosted and signed-update behavior remain PKG-002/M5 work.
