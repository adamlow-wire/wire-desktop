...
state: preparing-existing-local-protocol-candidate
...
This existing SEC-010 branch now reconciles its original protocol candidate with account/permission/managed-destination head `b46b0858`, preserving the already qualified source composition. PR #47 is still qualifying; no protocol PR is published yet. After #47 merges, reconcile protected integration, publish this scoped protocol PR and require its final-head gates. Original protocol baseline/implementation commits and proposed ADR 0003 remain in history.
