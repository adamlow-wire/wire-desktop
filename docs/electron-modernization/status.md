...
CAP-005 [draft PR #45](https://github.com/adamlow-wire/wire-desktop/pull/45) is stacked on PR #44, not integration. Implementation `ca1b8e22` passes 598 main tests (zero pending), 4 renderer, 97 React and 38 tools. Standalone diff against the preview dependency passes 39/47 statements (82.98%) and 23/23 security branches (100%); integration diff passes 215/227 statements (94.71%) and 137/144 branches (95.14%). Actual sandboxed metadata/navigation/cleanup tests pass 3/3 in 9.3 seconds on Linux. Separate link-policy and download-start sensitivity checks fail before restoration (the isolated start-check mutation produces one expected failure); both protections are restored. Native Windows junction coverage is wired into package baselines; a branch workflow-dispatch preflight is running. Retarget to integration after PR #44 merges and require applicable final-head gates before merging #45.
