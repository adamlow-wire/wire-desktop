...
A real two-account HTTP proxy fixture confirms a second CAP-005 defect: submitting the native prompt for the non-default account authenticates its request but changes the unrelated default session from `DIRECT` to the proxy. The clean failing product baseline is `e2e-tests/specs/regression/proxySession.spec.ts`; `/tmp/m3-proxy-session-baseline.log` records the exact unrelated-session assertion failure. Only synthetic loopback credentials are used. Baseline commit `5da7c871` is separate. Proxy application (system credentials and prompt submission) and cancellation reload now bind to the challenged web contents. The unchanged product target passes (5.2s); 67 native proxy/backend/IPC targets pass, and app/Mocha types and lint pass. Restoring the old main-window binding fails the exact cross-account assertion again, then the fix is restored. `/tmp/m3-proxy-session-{after,sensitivity,restored}.log` and `/tmp/m3-cap005-proxy-backends-native.log` retain evidence. The product target is added to the native three-platform gate; hosted evidence remains pending. The new fixture environment type is corrected; standalone Playwright checking retains only the nine existing errors.

A compatibility/security-target fixture now launches the normal application executable from an external Node driver, using only an isolated loopback page and fresh profile. It checks main-owned account identity, the immutable configuration value and denied page Node globals without inspector access, injected preload, product test flags or relaxed fuses. The native/package workflow runs the packaged executable before and after a test-owned App-lock policy value is installed through the actual OS backend (Linux file, Windows per-user registry, macOS app defaults), then removes only that value. Existing policy is never overwritten. This covers backend delivery/restart and an authenticated HTTP proxy path, not MDM deployment. The managed launch receives a real 407 proxy challenge and uses the normal native environment-credential path, with synthetic credentials confined to the child environment. Native platform/package results remain pending. The external proxy mode passes locally; canceling the actual native authentication callback prevents startup and fails the fixture, then source is restored/rebuilt. `/tmp/m3-packaged-proxy-{local,sensitivity,restored}.log` records this evidence. Local driver qualification passes against the rebuilt development app; removing the actual configuration exposure fails the expected assertions and restoration passes. A false managed-value expectation also fails. Commands/logs are recorded in testing.md. Normal shutdown is covered separately by the account fixture; this external startup driver terminates only its own process tree during cleanup.

PR #47 is now at `78231f74`, with explicit normal native-quit/exit assertions in the metadata fixture and metadata/restart added to its early platform gate after Windows worker-teardown failures at `b46b0858`. Those metadata changes and all three early product targets are now reconciled into this existing candidate. PR #47 passes build/lint/analysis and [all platform gates](https://github.com/adamlow-wire/wire-desktop/actions/runs/34833701114); Windows storage/metadata gives two passes without retry. Its [full E2E run](https://github.com/adamlow-wire/wire-desktop/actions/runs/34834177940) remains in progress. This candidate will use workflow dispatch for early native/package qualification while that dependency completes, then still requires its final integration-base PR gates. No final-head gate is inherited.
