...
state: preparing-existing-second-instance-candidate
...
This existing CAP-006 branch reconciles its original secondary-process candidate with account/permission/managed-destination/protocol code. The child now uses the current short profile. Source matches the already qualified combined delta: 52 native cases and the real Linux two-process storage/restart fixture pass. PR #47 and the protocol dependency still require integration before publishing this scoped lifecycle PR; native Windows proof remains required.
...
| Managed backends `b4a4ee69` | Existing representative adapters and string-hive typing are reconciled with `b46b0858`. A real native Windows test-owned registry-to-destination check and CI guard sensitivity are prepared. Build/types/lint and 45 Linux native cases pass; Windows evidence remains required. |
