...
An **unwired permission policy** now implements separate audio/video/notification grants, exact registered view/session/origin checks, main-frame-only authorization, foreground eligibility for new consent, one pending request, and document-generation revocation. The [proposed consent ADR](./decisions/0002-account-permission-consent.md) records boundaries before integration. Ten simulated policy targets plus three native baselines pass; the policy alone has 44/44 statements and 48/48 branches covered in a clean focused run. Removing the generation check makes a late answer grant after revocation and fails its target; restored. App/Mocha types and changed-source lint pass. Evidence: `/tmp/sec009-policy-focused-coverage.log`, `/tmp/sec009-policy-sensitivity.log`, `/tmp/sec009-policy-*-types.log`. This focused run is **not** aggregate product coverage. Native consent, automatic lifecycle binding, media/display probes and platform qualification are not implemented by this checkpoint.
...
1. SEC-009: add the native request/check adapter with fail-closed callback/error handling and lifecycle revocation; characterize media/display callback paths with fake devices. Integrate bounded main-owned consent only after the proposed ADR's user-flow/lifecycle gates have evidence. Keep unknown permissions, unknown senders and subframes denied; no automatic origin-wide grants. Scope/identity/revocation policy is tested but remains unwired.
...
| --- | --- | --- |
| Electron main | 754 passing with SEC-009 baseline/policy tests; production remains deny-all | `/tmp/sec009-policy-full-main.log` |
