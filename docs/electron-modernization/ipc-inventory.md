...
| `wire-desktop:desktop-sources:enumerate:v1` | none | Legacy thumbnail enumeration | CAP-003 local candidate removes its production binder and legacy bridge/global; helper characterization remains historical. New source lists stay inside the local broker | SEC-003, CAP-003 |
| `wire-desktop:display:begin:v1` | exact owning account or parent-bound PiP | Opens a local source chooser; no source authority in request | Local qualification: exact v4 request ID, registered capability/origin/main frame, foreground/owner checks, 4/minute and one flow/account; response is a main-generated flow ID | CAP-003 |
| `wire-desktop:display:stop:v1` | exact flow owner | Ends capture | Local qualification: exact flow ID, current sender authorization and identity equality; cross-account denial, idempotent missing flow | CAP-003 |
| `wire-desktop:display:broker:read:v1` | exact fixed local broker document | Main-owned source names/thumbnails shown only in broker | Local qualification: no payload, bounded validated source/locale model, one read per flow, no remote source identifiers | CAP-003 |
| `wire-desktop:display:broker:select:v1` | exact fixed local broker document | One-use native video capture grant | Local qualification: opaque choice ID mapped only by main, foreground broker, current owner and pending phase; direct remote media permission stays denied | CAP-003 |
| `wire-desktop:display:broker:stop:v1` | exact fixed local broker document | Stops its owning capture | Local qualification: no payload, registered broker-to-flow mapping, native window/track disposal | CAP-003 |
| `wire-desktop:display:broker:heartbeat:v1` | exact active local broker document | Renews bounded main-process capture deadline after successful delivery | Local qualification: no payload, current owner and active/starting phase, 90/minute; ten-second watchdog also ends hung brokers | CAP-003 |
...
CAP-003 also transfers main-created message ports to the exact authorized requesting main frame and its isolated broker. The remote endpoint receives bounded contiguous RGBA and acknowledges one frame at a time; it cannot choose sources or invoke main privileges through the port. Static screens repeat the latest approved frame at at most5fps. Limits are3840×2160, one cached image plus one in-flight image, a ten-second delivery/watchdog deadline and sixty-second source choice. Main-to-renderer ending notification revokes original and cloned generated tracks. These are local candidate contracts; final native/product/platform evidence is still required.
