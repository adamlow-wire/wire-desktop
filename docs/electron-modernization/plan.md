...
- September 24 F-035 reopening: [test-only baseline](https://github.com/adamlow-wire/wire-desktop/actions/runs/36063921163) proves a committed cross-origin child with `noopener noreferrer` opens an external URL on Windows, macOS and Linux despite the prior policy. The first grant candidate [run `36067600554`](https://github.com/adamlow-wire/wire-desktop/actions/runs/36067600554) denies that child and preserves PiP/SSO, but fails ordinary top-level native anchor and deep-link positives on all three platforms; it is rejected. The revised native-anchor grant at `3386c9e1` passes [hosted run `36068535525`](https://github.com/adamlow-wire/wire-desktop/actions/runs/36068535525) on Windows, macOS and Linux, including selected native popup allow/deny tests, product account/calling checks and full unsigned package jobs. Final composition/re-review and full handoff gates remain open.
