...
- Local checkpoint: shell, About and proxy prompt now use the per-session finite-resource handler, with six native transport targets and 2/2 Linux product lifecycle/metadata restart fixtures passing. New URL/registration assertions fail on prior builds. Production/development custom-scheme ordinary-script CSP tests pass and are sensitivity-proven; only standard/secure scheme privileges are enabled. Source audit retains just the script-disabled legacy importer as a file-origin document loader. Review, aggregate coverage and final platform gates remain open; the latest full native run retains one known tray-focus timeout. No SEC-010 closure or merge is claimed.
...
| --- | --- | --- | --- | --- |
| 1.5.27 | 2026-09-10 | Codex | Completed local auxiliary scheme cutover, custom-scheme production/development CSP probes and minimal privilege configuration; retained legacy import and all review/coverage/platform gates | SEC-010, DEC-010, CAP-001 |
