...
- September 24 F-035 follow-up: the foreign-child empty-referrer bypass fails its deny baseline on all three OSes at `e983f58a`. The first scoped candidate broke top-level chat/deep links despite denying the child. [Fork draft PR #71](https://github.com/adamlow-wire/wire-desktop/pull/71) corrects that path; `3386c9e1` passes [all-platform native/product/unsigned-package run `36068535525`](https://github.com/adamlow-wire/wire-desktop/actions/runs/36068535525). Keep SEC-008 open until final composed re-review and qualification.
