...
- Evidence: See [review findings](review-findings.md) and [actual/corrected Linux ASAR and installer audit](shipped-linux-asar-audit.md#actual-unsigned-linux-installers-from-the-composed-candidate). The combined candidate passes local unsigned Linux directory plus actual AppImage/deb/rpm static content, dependency-inventory and fuse checks; that first installer trio had mixed local build identity and does not qualify environment separation. F-021 confirms the original AppImage desktop and generated launcher disabled Chromium sandboxing; baseline `84ac22aa` and scoped fix `45f45f4f` remove those defaults. An environment-correct internal AppImage at `7771a714` passes static identity, launcher, ASAR/dependency and fuse checks; native and hosted qualification remain open. Packaged native startup, reproducible CI, Windows/macOS artifacts and final all-platform qualification remain pending.
