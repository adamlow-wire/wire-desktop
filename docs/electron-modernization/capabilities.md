...
| DCP-013 | Managed configuration | Windows, macOS, Linux | Central managed-config tests plus an account-only, origin-bound, schema-validated read contract; platform backends and packaged managed-device behavior remain untested | low | BASE-002 | CAP-005 | Backend fixtures, final isolated immutable bridge, and packaged managed-device tests |
