...
| DCP-003 | Enterprise and automated SSO | Windows, macOS, Linux | Characterization includes a deterministic Electron fixture, exact SSO origin/session/account view identity, and account-owned close/focus controls with cross-account denial; three CAP-002 security targets quarantined | medium | TST-002 | CAP-002 | Controlled IdP E2E, protocol abuse tests, cookie/account isolation, cleanup |
...
| DCP-005 | Tray and native application menu | Windows, macOS, Linux | Sensitivity-proven tray click, tooltip, unread/flashing, icon selection, and macOS/non-macOS platform tests; menu E2E and all-platform hosted package validation | medium | TST-003 | CAP-004 | Packaged native tray/menu/tooltip smoke |
...
| DCP-009 | Picture-in-picture call windows | Windows, macOS, Linux | Exact account, origin, URL, session, and lifecycle identity tests | low | BASE-002, TST-004 | CAP-003 | Fixed preferences, navigation policy, lifecycle and interaction tests |
| DCP-010 | Deep links and single-instance routing | Windows, macOS, Linux | Happy-path dispatch and conversation-join parameter tests plus an application-shell-only, exact, bounded, rate-limited submission contract; parser and lifecycle policy remain open | low | TST-004 | CAP-006, SEC-013 | Valid lifecycle routing plus malformed, oversized, recursive, and hostile input |
| DCP-011 | Proxy configuration and authentication | Windows, macOS, Linux | Proxy parsing/generation plus exact prompt identity, sender authorization, bounded credentials, one-shot submit/cancel coordination, retry, session policy, and reload behavior tests | medium | BASE-002 | CAP-005 | Packaged authenticated-proxy smoke per OS and enterprise proxy variants |
...
| DCP-013 | Managed configuration | Windows, macOS, Linux | Central managed-config tests plus account-only, origin-bound, schema-validated managed-config reads and enforced-download-location updates; Windows directory/persistence behavior is sensitivity-proven, while path containment, platform backends, and packaged managed-device behavior remain untested | low | BASE-002 | CAP-005 | Backend fixtures, download-path policy, final isolated immutable bridges, and packaged managed-device tests |
| DCP-014 | External links and context menus | Windows, macOS, Linux | Protocol filtering plus an account-only, size-bounded, rate-limited native save-picture contract; remaining context-menu paths have limited direct coverage | low | TST-004 | SEC-004, SEC-008, SEC-013 | Allowed/denied protocol tests, complete isolated context-menu bridge, and packaged OS-browser/save smoke |
| DCP-015 | Open Graph retrieval | Main process | Sensitivity-proven metadata behavior plus account-only sender authorization, exact bounded request schema, response validation, and a per-view quota; destination policy remains open | low | TST-004 | SEC-012 | SSRF, DNS/redirect, protocol, credential, timeout, and response-byte tests |
