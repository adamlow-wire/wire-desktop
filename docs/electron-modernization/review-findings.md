...
| F-011 | High pending picture-save retention / SEC-003, SEC-004 | Real save-picture binder with valid identity, one-byte payloads, controlled clock and unresolved native-boundary promises admits 50 simultaneous save operations across five quota windows. Production awaits a native save dialog while retaining up to 15 MiB per request. | Open: bound in-flight native save work independently of the per-minute quota; preserve allow/deny and failure/recovery behavior with sensitive tests. No large allocation or native dialog was used in reproduction. |
