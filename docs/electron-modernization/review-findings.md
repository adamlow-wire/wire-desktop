...
| F-009 | Candidate sensitive SSO diagnostics / CAP-002, INV-010 | Native SSO load URL contains a callback secret; initialization errors reach a raw-error logger. SSO console forwarding also lacks redaction. | Open: reproduce with synthetic values, assess native error contents and narrow remediation. No exposure incident asserted. |
