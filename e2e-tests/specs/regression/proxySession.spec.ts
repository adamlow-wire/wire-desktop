...
test('[CAP-005][security-target] proxy authentication changes only the challenged account session', async () => {
  ...
  try {
    ...
    await expect.poll(() => app!.windows().some(page => page.url().includes('proxy-prompt.html'))).toBe(true);
